...
runtimeException :: (a -> b -> RuntimeException) -> a -> b -> Result
runtimeException f a b = Left . RuntimeException $ f a b

arityException :: Int -> Int -> Result
arityException = runtimeException ArityException

typeError :: [SchemeType] -> SchemeType -> Result
typeError = runtimeException TypeError
...
lookupEnv (Cons (Cons x _) e) s = typeError [SymbolType] (typeOf x)
lookupEnv (Cons x e) s = typeError [ConsType] (typeOf x)
lookupEnv x s = typeError [ConsType] (typeOf x)

eval :: Env -> Value -> Result
eval _ Nil = return Nil
eval e (Symbol "*env*") = return e
eval e s@(Symbol _) = lookupEnv e s
eval e (Lambda f source) = typeError [NilType, ConsType, SymbolType] LambdaType
eval e c@(Cons a d) =
  case a of
...
schemeFn f = Lambda g nativeCode
  where g [x] = f x
        g args = arityException 1 (length args)

schemeFn2 :: (Value -> Value -> Result) -> Value
schemeFn2 f = Lambda g nativeCode
  where g [x, y] = f x y
        g args = arityException 2 (length args)
...
consFn f = schemeFn g
  where g c@(Cons _ _) = return (f c)
        g x = typeError [ConsType] (typeOf x)
